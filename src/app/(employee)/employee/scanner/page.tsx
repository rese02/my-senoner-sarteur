...
function ScanResultView({ user, onNextCustomer }: { user: UserType, onNextCustomer: () => void }) {
    ...
    const handleAddStamp = () => {
        ...
        toast({ title: 'Stempel hinzugefügt!', description: `${user?.name} hat jetzt ${loyaltyData.stamps + 1} Stempel.` });
    };
    ...
    return (
         <Card className="w-full text-center shadow-xl animate-in fade-in-50">
            <CardHeader>
                ...
                <CardDescription>Aktuelle Stempel: {loyaltyData?.stamps}</CardDescription>
            </CardHeader>
