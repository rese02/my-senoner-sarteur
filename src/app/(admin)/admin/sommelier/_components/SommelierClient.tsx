...
import { Trash2, Save, Loader2, FileWarning, Edit } from 'lucide-react';
import { bulkImportWines, deleteAllWines, deleteWine, updateWine } from '@/app/actions/wine-manager.actions';
...
export function SommelierClient({ initialWines }: { initialWines: Product[] }) {
  ...
  const [editingWine, setEditingWine] = useState<Product | null>(null);
  ...
  const handleOpenEditModal = (wine: Product) => {
      setEditingWine(wine);
  ...
  const handleUpdateWine = () => {
      ...
      startTransition(async () => {
          try {
            const updatedWine = await updateWine(editingWine);
            setWines(current => current.map(w => w.id === updatedWine.id ? updatedWine : w));
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={handleDeleteAll} className="bg-destructive hover:bg-destructive/90">Ja, alles löschen</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
              </CardContent>
        ...
        <DialogContent className="sm:max-w-md p-0">
          ...
          {editingWine && (
            <div className="grid gap-4 p-6">
                ...
                <div className="space-y-1.5">
                    <Label htmlFor="wine-tags">Tags (Komma-getrennt)</Label>
                    <Textarea
                        id="wine-tags"
                        value={editingWine.tags?.join(', ')}
                        onChange={(e) => setEditingWine({...editingWine, tags: e.target.value.split(',').map(t => t.trim())})}
                        placeholder="rotwein, kräftig, fleisch, käse"
                    />
                </div>
            </div>
          )}
